...
# df_all = df_all[~df_all["graph_name"].str.contains("padgett_florence_families")]

# %%
sns.lineplot(data=df_all, x="time_category", y="final_num_lang", marker="o")
plt.title("Average Final Number of Languages over Time")
plt.xticks(ticks=list(time_mapping.values()), labels=list(time_mapping.keys()), rotation=45)
plt.show()

# %%
sns.lineplot(data=df_all, x="time_category", y="final_num_lang_time", marker="o")
plt.title("Average Time to Final Language over Time")
plt.xticks(ticks=list(time_mapping.values()), labels=list(time_mapping.keys()), rotation=45)
plt.show()
